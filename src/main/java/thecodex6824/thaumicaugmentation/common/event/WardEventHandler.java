...
public class WardEventHandler {
    ...
    public void onBlockPunch(PlayerInteractEvent.LeftClickBlock event) {
        ...
        IWardStorage storage = chunk.getCapability(CapabilityWardStorage.WARD_STORAGE, null);
        if (storage != null) {
            if(WardHelper.doesEntityHaveSpecialPermission(event.getEntityPlayer())){
                MinecraftForge.EVENT_BUS.post(new BlockEvent.BreakEvent(event.getWorld(),pos,event.getWorld().getBlockState(pos), event.getEntityPlayer()));
                return;
            }
